...
class Component extends Component_helper {
  ...
  */
  public static function create_assets() {
    ...
    $css_path = get_stylesheet_directory() . '/os-base-theme.min.css';
    $js_path = get_stylesheet_directory() . '/js/os-base-theme.min.js';
    ...
    if (($build_css || $build_js) && $cp_cache = self::get_cp_cache()) {
      $css = $js = "/*! OS Base Theme | by Oddessey Solutions | oddesseysolutions.nl */";
      ...
      if ($build_css) file_put_contents(get_stylesheet_directory() . '/os-base-theme.min.css', $css);
      if ($build_js) file_put_contents(get_stylesheet_directory() . '/js/os-base-theme.min.js', $js);
      return true;
    }
    return false;
  }
